...
import 'package:relife/ui/pages/view_habit/update_view_habit_page.dart';
...
class UpdatePlanAccountabilityPage extends StatefulWidget {
  final Detail detail;
  const UpdatePlanAccountabilityPage({Key? key, required this.detail})
      : super(key: key);

  @override
  State<UpdatePlanAccountabilityPage> createState() =>
      _UpdatePlanAccountabilityPageState();
}

class _UpdatePlanAccountabilityPageState
    extends State<UpdatePlanAccountabilityPage> {
  @override
  void initState() {
    WidgetsBinding.instance!.addPostFrameCallback((_) async {
      final updateHabit =
          Provider.of<UpdateHabitsProvider>(context, listen: false);
      updateHabit.setDaysPerMonth(
          widget.detail.daysPerMonth.toString() + ' days per month');
      updateHabit.setPunishment(widget.detail.punishment);
      updateHabit.setFriends(widget.detail.accountabilityPartnerName);
      updateHabit
          .setPhone(widget.detail.accountabilityPartnerPhoneNumber.toString());
              ...
              child: Column(
                ...
                children: [
                  ...
                  _buildParagraph(
                    ...
                    onTap: () async {
                      await addHabit.getContacts();

                      buildBottomSheetRecommendationFriends(
                        context: context,
                        heading: "select friends",
                        // recommended: contactSearchController.text.isEmpty
                        //     ? addHabit.contacts
